...
BEGIN
  ...
  IF (:varCounter = 0) THEN
     varStatement := '';
  ELSE
     varStatement := 'CREATE JWT PROVIDER JWTPROVIDER_465C997C_7F6A_41F4_B599_0677FDAFF029 WITH ISSUER ''http://p2001064106trial.localhost:8080/uaa/oauth/token'' CLAIM ''user_name'' AS EXTERNAL IDENTITY';
     EXEC :varStatement;
  END IF;
END;
--
...
BEGIN
  ...
  IF (:varCounter = 0) THEN
     varStatement := '';
  ELSE
     varStatement := 'ALTER JWT PROVIDER JWTPROVIDER_465C997C_7F6A_41F4_B599_0677FDAFF029 SET ISSUER ''http://p2001064106trial.localhost:8080/uaa/oauth/token'' CLAIM ''user_name'' AS EXTERNAL IDENTITY';
     EXEC :varStatement;
  END IF;
END;
--
